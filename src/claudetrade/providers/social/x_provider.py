...
#
# CORRECTION (2026-08-02): do not read that empty-bodied 404 as evidence about
# the query ID. Measured against the owner's live session, a CURRENT ID and a
# garbage one return byte-identical bodiless 404s, and a genuine Chrome TLS
# fingerprint fares no better -- x.com now refuses non-browser callers at the
# edge, before the ID is ever considered. The error path below says so. The
# earlier note above is kept because the login-wall misreport was real, but its
# "therefore the ID is stale" inference was not.
...
#: Feature switches, copied verbatim from a real logged-in browser capture of
#: a ``$MSFT`` cashtag search (owner capture, 2026-08-02). X validates this set
#: against the one the query ID was registered with and rejects a request that
#: does not carry it -- sending a curated "just the ones we need" subset is NOT
#: a valid request, which is how this adapter previously shipped: a single flag
#: here produced a rejection the status checks below then attributed to a stale
#: query ID, sending operators off to re-capture an ID that was already
#: current. Refresh this block the same way it was captured (devtools ->
#: Network, filter "graphql", search any cashtag, copy the decoded ``features``
#: parameter whole). Do not prune, reorder or hand-edit individual flags to
#: match what the parser happens to read -- they are X's switches, not ours.
...
class XProvider:
    ...
    def _fetch_session_query(
            ...
            "variables": json.dumps(
                {
                    ...
                    # "Latest" (not the browser capture's "Top"): this adapter
                    # wants reverse-chronological posts for a sentiment window,
                    # and both are values the same operation accepts.
                    "product": "Latest",
                    # Carried from the same capture as _SEARCH_GRAPHQL_FEATURES.
                    # The operation declares these variables, so omitting them
                    # is as invalid as omitting a feature switch, even though
                    # neither affects what this adapter reads back.
        ...
        if response.status_code >= 400:
            # A 404 carrying NO content-type and a ZERO-length body is not this
            # adapter getting the request wrong -- it is x.com's edge refusing
            # non-browser access to /i/api/graphql/* outright. Measured
            # 2026-08-02, all against the owner's live session:
            #   * a valid query ID and a garbage one return byte-identical
            #     bodiless 404s, so the status says nothing about the ID;
            #   * a real Chrome TLS fingerprint (curl_cffi impersonate=chrome)
            #     returns the same bodiless 404 as httpx, so it is not TLS
            #     fingerprinting either;
            #   * the same cookies against a v1.1 REST endpoint get a normal
            #     JSON error body, so the cookies still reach the application;
            #   * https://x.com/ itself returns 200, so egress is fine.
            # Do not "fix" this by re-capturing the query ID or widening the
            # feature set -- both were tried and changed nothing. Getting past
            # it would mean reproducing x.com's client-side anti-automation
            # token, which ADR-0008 Decision 1 forbids outright ("never
            # rotates a fingerprint or proxy to work around a block"). Failing
            # closed here IS the designed behaviour.
            bodiless = not response.headers.get("content-type") and not response.content
            if response.status_code == 404 and bodiless:
                raise SourceBlockedError(
                    f"X refused the session search for '{cashtag}': HTTP 404 with no "
                    "content-type and an empty body -- x.com's edge declining "
                    "non-browser access to its internal GraphQL API, not a problem "
                    "with your configuration. Your query ID, feature set and cookies "
                    "are NOT the cause: a valid ID and a garbage one get the same "
                    "bodiless 404, and the same cookies still work against other X "
                    "endpoints. Re-capturing the query ID will not help. Fail-closed "
                    "per ADR-0008 Decision 1; the supported route to X data is the "
                    "official API v2 -- set x.bearer_credential -- or leave X off "
                    "with x.session_enabled = false.",
                    provider="x",
                )
            raise SourceBlockedError(
                ...
                "above), so this is the request line itself: either the configured "
                "query ID has gone stale, or the features/variables no longer match "
                "the set that query ID expects (see _SEARCH_GRAPHQL_FEATURES). "
                "Compare BOTH against one browser capture before changing either. "
                f"{_QUERY_ID_CAPTURE_HINT}",
                provider="x",
            )
