...
| `get_adanos_budget()` | read-only, free | Current official-API budget state: used/remaining of the ~250/month free tier, reserve floor, whether the `adanos_api_key` resolves. Costs nothing. |
| `get_adanos_detail(symbol, platform)` | read-only, **spends quota** | Per-ticker Adanos detail (daily trend, sentiment breakdown, top posts/authors). **Spends 1 official-API request of the ~250/month free tier** unless served from the same-session enrichment cache (`from_cache: true`). Refuses cleanly when no key is configured or the budget has hit its reserve. |
| `get_adanos_explain(symbol, platform)` | read-only, **spends quota** | Adanos's AI-generated explanation of why the ticker is trending. Same budget guard; the vendor caches explanations ~6 h server-side. |
