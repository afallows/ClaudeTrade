...
| `get_analyst_sentiment(symbol)` | read-only | Latest TipRanks analyst snapshot + delta vs the previous stored session: ranked-consensus Buy/Hold/Sell counts, coverage changes, price-target consensus (mean/high/low) and its moves, recent per-analyst rating actions with analyst quality, and earnings surprise. Harvested from data the daily refresh already downloads — no extra vendor calls. |
| `get_institutional_sentiment(symbol)` | read-only | Weighted insider/hedge-fund score in [-1, +1] with the axis breakdown (insider flow + confidence vs hedge-fund sentiment + holdings trend), staleness ages and applied weights, recent insider transactions (with SEC links) and notable holder moves. `None` when no institutional data exists — never a fabricated neutral. |
| `get_adanos_budget()` | read-only, free | Current official-API budget state: used/remaining of the ~250/month free tier, reserve floor, whether the `adanos_api_key` resolves. In normal operation nothing spends this budget — it's a durability reserve behind the free site endpoints. |
| `get_adanos_detail(symbol, platform)` | read-only, usually free | Per-ticker Adanos detail (daily trend, sentiment breakdown, top posts/authors) via the site-first ladder: the keyless site proxy answers normally (`mode: "site"`, `quota_spent: false`); the metered official API is only the fallback. Same-session enrichment cache hits cost nothing at all. |
| `get_adanos_explain(symbol, platform)` | read-only, usually free | Adanos's AI-generated explanation of why the ticker is trending, site-first like detail. Polymarket has no explain endpoint on the site proxy — that platform needs the official key. Vendor caches explanations ~6 h server-side. |
| `get_adanos_market_sentiment(platform)` | read-only, usually free | Market-wide sentiment snapshot for one platform: overall buzz/trend, active tickers, bullish/bearish split, 7-point history, and the top drivers moving the market's mood. A natural first call for a morning briefing, alongside `get_market_status`. |
