...
### Free or low-cost equivalents (verify terms before use)

“Free” does not mean real-time, complete, licensed for redistribution, or safe
for historical validation. Plan and entitlements change; check the provider's
official pricing, market-data agreements, and documentation on the day you
configure it.

| Need | Possible authorized source | Practical use and limitations |
|---|---|---|
| Offline research | User-supplied CSV | Select `provider = "csv"`; retain provenance and the source licence. This is the safest no-network fallback. |
| Free daily history | Stooq | Select `provider = "stooq"`; no key. Daily, unadjusted here, not real-time, no point-in-time/delisted universe, and no assumed redistribution rights. |
| US real-time research | Alpaca Market Data | A free account/API key may provide IEX-sourced real-time US data, while full-market SIP coverage and historical depth depend on the current plan. **No adapter is implemented yet**; implement `MarketDataProvider`, store credentials in the keyring, and label the feed `IEX` rather than “whole market.” |
| Limited API experimentation | Alpha Vantage | A free API key may offer limited request volume; real-time or delayed US data may require a premium entitlement. **No adapter is implemented yet.** Do not use it until adjusted-price, timestamp, throttling, and licence behavior have contract tests. |
| Brokerage-linked quotes | User's authorized broker | Often the best lawful source for the user's own display, but entitlements and redistribution rules apply. No broker adapter is present and live trading remains disabled. |

No free provider listed above solves survivorship bias, delisted coverage,
point-in-time fundamentals, full-market real-time quotes, and commercial
redistribution in one feed. Do not silently substitute delayed or IEX-only
quotes for consolidated real-time market data.
...
- The current adapter collects submissions, not comments; `comments_per_post` is reserved and currently unused
- No pagination or historical engagement snapshots are implemented
...
### Are there free Reddit or X equivalents?

* **Reddit:** the official Reddit API is the supported connector. Whether access
  is free or needs approval depends on the current use, volume, and commercial
  status. Configure OAuth as above; do not substitute Pushshift, scraping, or
  browser automation without explicit authorization and compatible licensing.
* **X:** there is no endorsed free drop-in source for recent-search data in this
  project. If the official plan does not permit the required reads, set
  `enabled = false`. The registry continues with Reddit and/or synthetic data,
  and downstream confidence should be reduced. Do not scrape X or bypass its
  access controls.
* **Cross-platform substitutes:** StockTwits or other licensed finance-social
  APIs could be added behind `SocialProvider`, but their content is not
  equivalent to Reddit or X and must remain a separately scored source. No such
  adapter is implemented or verified in this repository.

The operator must re-check official terms and pricing before deployment. This
document describes integration behavior, not a promise of free access.
